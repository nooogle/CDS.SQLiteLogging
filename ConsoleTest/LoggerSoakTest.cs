...
sealed class LoggerSoakTest
{
    ...
    public LoggerSoakTest(string folder, int entriesPerSecond = 10)
    {
        this.entriesPerSecond = entriesPerSecond;

        // Use aggressive housekeeping settings for the soak test
        var batchingOptions = new BatchingOptions
        {
            FlushInterval = TimeSpan.FromMilliseconds(2000),  // Flush every two seconds
            MaxCacheSize = 1000,        // Up to 1000 entries per batch
            BatchSize = 100             // Write up to 100 entries per batch
        };

        var houseKeepingOptions = new HouseKeepingOptions
        {
            RetentionPeriod = TimeSpan.FromMinutes(1),     // Delete entries older than 1 minute
    ...
    /// <summary>
    /// Runs the soak test with user-defined parameters from the console.
    /// </summary>
    public static void RunFromConsole()
    {
        // Get the non-roaming application data folder
        string folder = LogFolderManager.GetLogFolder(nameof(LoggerSoakTest));

        // Display header
        Console.Clear();
        Console.WriteLine("==== SQLite Logger Soak Test ====");
        Console.WriteLine();
        Console.WriteLine($"Log folder: {folder}");
        Console.WriteLine();
        Console.Write("Enter entries per second (default 10): ");
        string rateInput = Console.ReadLine();
        int entriesPerSecond = string.IsNullOrWhiteSpace(rateInput) ? 10 : int.Parse(rateInput);

        var soakTest = new LoggerSoakTest(folder, entriesPerSecond);
        soakTest.Run();
    }
}
